...
const diceCountDownAnimation = document.getElementById(
  'end-game-countdown-animation'
);
...
function rollFourDiceAndDisplay() {
  ...
  $playerScoreThisRound.html(
    `Score This Round: <strong>${playerScorePerRound}</strong>`
  );
  $computerScoreThisRound.html(
    `Score This Round: <strong> ${computerScorePerRound} </strong>`
  );
  ...
  $playerTotalScoreDisplay.html(
    `Total Score: <strong>${playerArraySum}</strong>`
  );
  $computerTotalScoreDisplay.html(
    `Total Score: <strong>${computerArraySum}</strong>`
  );
  ...
  function endGamePopUp() {
    ...
    $('#player-final-score-display').html(
      `<strong>Your Final Score: ${playerArraySum} </strong>`
    );
    $('#computer-final-score-display').html(
      `<strong> Computer's Final Score: ${computerArraySum} </strong>`
    );

    if (playerArraySum > computerArraySum) {
      gameWinAudio.play();
      $endGameWinner.html(
        `You win! You have won by ${
          playerArraySum - computerArraySum
        } point(s) with a total score of ${playerArraySum}.`
      );
      $('#end-game').css('background-color', '#4FB286');
    } else if (playerArraySum < computerArraySum) {
      gameLoseAudio.play();
      $endGameWinner.html(
        `You lose! The Computer has won by ${
          computerArraySum - playerArraySum
        } point(s) with a total score of ${computerArraySum}.`
      );
